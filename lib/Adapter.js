...
function AdapterCore(mainGroup, config, swarmComImpl) {
    ...
    if(!swarmComImpl){
        swarmComImpl = require("../com/PSS.js").implemenation;
    }
    ...
    function onMessageFromQueue(initVars) {
        ...
        if (swarmingPhase.meta.debug == true) {
            logger.info("[" + thisAdapter.nodeName + "] executing message: \n" + M(initVars));
        }
    ...
    var fb = require("../com/old/FileTransferBusHttpImpl.js").initFileBusNode;
...
require("safebox").container.service("guaranteed_startSwarm", ['swarmingIsWorking'], function(outOfService, swarming){
    if(!outOfService){
        var savedPending = pendingSwarms;
        pendingSwarms = null;
        if(savedPending.length){
            console.log(thisAdapter.mainGroup, ": Restarting pending swarm...");
        }
...
startSwarm = function (swarmingName, ctorName) {
    if(pendingSwarms){
        cprint(thisAdapter.mainGroup, ": Saving pending swarm...");
    ...
    if(throttler.accept(function(){
            try {
                ...
                if(global_swarmSystem_config.Core.performSwarmAnalytics){
                    try{
                        performAnalytics(swarming.meta.userId,swarmingName,ctorName,swarming.meta,args)
                    }catch(err)
                    {
