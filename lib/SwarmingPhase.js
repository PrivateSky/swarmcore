...
SwarmingPhase.prototype.broadcast = function (phaseName,groupName) {
    ...
    var nodes = swarmComImpl.getGroupNodes(groupName);
    var self = this;
...
SwarmingPhase.prototype.swarm = function (phaseName, nodeHint) {
    var mySwarm = util.newSwarmPhase(this.meta.swarmingName, phaseName , this);
    try {
        mySwarm.meta.currentPhase = phaseName;
        var targetNodeName = nodeHint;
        if (nodeHint == undefined) {
            var phase = CompiledSwarmRepository.prototype.getSwarmDescription(mySwarm.meta.swarmingName)[phaseName];
            if (phase == undefined) {
        ...
        logErr("Unknown error in phase {" + phaseName + "} nodeHint is {" + targetNodeName + "} Dump: " + J(CompiledSwarmRepository.prototype.getSwarmDescription(mySwarm.swarmingName)), err);
    }
};
