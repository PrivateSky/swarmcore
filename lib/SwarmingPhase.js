...
SwarmingPhase.prototype.swarm = function (phaseName, nodeHint , honeyRequest, outlet) {
    ...
    try {
        ...
        if (mySwarm.meta.debug == true) {
            logger.info("Starting swarm " + this.meta.swarmingName + " towards " + swarmTarget + ", Phase: " + phaseName);
        }
...
SwarmingPhase.prototype.result = function (result) {
    this.__result = result;
    this.swarm("*", this.getEntryAdapter(), true);
}
...
SwarmingPhase.prototype.shareFile = function (path,failurePhaseName,  successPhaseName, targetNode) {
    var self = this;
    thisAdapter.nativeMiddleware.shareFile(path, function(err, transferId){
        self.__transferId = transferId;
        var targetPhase = successPhaseName;
        if(err){
            targetPhase = failurePhaseName;
        }
        continueSwarm(self, targetPhase, targetNode);
    })
}


SwarmingPhase.prototype.download = function (transferId, filePath, callback) {
    thisAdapter.nativeMiddleware.download(transferId, filePath, callback);
}

SwarmingPhase.prototype.unshare = function (transferId, callback) {
    thisAdapter.nativeMiddleware.unshare(transferId, callback);
}
